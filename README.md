...
실제 운영처럼 지식베이스는 계속 누적되지만 현재 질문의 정답 라벨은 바로 얻을 수 없다고 가정했습니다. 먼저 synthetic fixture와 작은 temporal mock으로 데이터, 검색, 생성, 탐지 단계가 끝까지 연결되는지 확인했습니다. 이후 T0에서 detector와 threshold를 고정하고 미래 업데이트로 넘겨, 실험 중에 기준을 다시 맞추지 않도록 했습니다.

탐지 성능이 좋아도 원인을 설명하지 못하면 운영자가 다음 조치를 정하기 어렵습니다. 그래서 고위험 사례를 다시 살펴보고 P1-P5 evidence intervention을 추가했습니다. 실험 결과와 함께 약한 구간, 해석 한계, 재현 절차와 운영화에 필요한 다음 학습 항목을 별도 문서로 남겼습니다.
